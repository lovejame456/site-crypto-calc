var ChartManager = (function () {
  ...
  function updateMainChart(dates, prices, ma, equity) {
    if (!mainChart) return;

    var priceData = prices.map(function (p, i) { return [dates[i], p]; });
    var maData = ma.map(function (v, i) { return v !== null ? [dates[i], v] : null; }).filter(Boolean);
    ...
    option.series = [
      ...
      },
      {
        ...
        data: maData,
        symbol: 'none',
        lineStyle: { color: '#ff3e6c', width: 1, type: 'dashed' }
      },
      {
        name: '策略净值',
        type: 'line',
